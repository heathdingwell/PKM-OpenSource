...
interface CreateStackDialogState {
  name: string;
}
...
interface StackRenameDialogState {
  oldName: string;
  newName: string;
}
...
const commandPaletteActions: CommandPaletteAction[] = [
  ...
  { id: "toggle-ai", label: "Toggle AI copilot pane", keywords: ["ai", "copilot", "assistant", "chat", "pane"] },
  ...
  try {
    ...
    return {
      ...
      shortcutNotebooks: Array.isArray(parsed.shortcutNotebooks)
        ...
        : [],
      shortcutTags: Array.isArray(parsed.shortcutTags)
        ? parsed.shortcutTags.filter((name): name is string => typeof name === "string")
        : [],
      ...
      notebookStacks:
        parsed.notebookStacks && typeof parsed.notebookStacks === "object"
          ...
          : {},
      customStacks: Array.isArray(parsed.customStacks)
        ? parsed.customStacks.filter((stack): stack is string => typeof stack === "string")
        : [],
...
export default function App() {
  ...
  const [stackRenameDialog, setStackRenameDialog] = useState<StackRenameDialogState | null>(null);
  ...
  const [shortcutTags, setShortcutTags] = useState<string[]>(initialPrefs.shortcutTags ?? []);
  ...
  const stackNames = useMemo(() => {
    return Array.from(new Set([...customStacks, ...Object.values(notebookStacks)].map((name) => name.trim()).filter(Boolean)))
      .sort((left, right) => left.localeCompare(right));
  }, [notebookStacks, customStacks]);
  ...
  const stackedNotebookGroups = useMemo(() => {
    ...
    const stacks = stackNames.map((stack) => ({
      stack,
      notebooks: (groups.get(stack) ?? []).sort((a, b) => a.localeCompare(b))
    }));

    return { stacks, unstacked: unstacked.sort((a, b) => a.localeCompare(b)) };
  }, [notebookList, notebookStacks, stackNames]);
        ...
        : browseMode === "templates"
          ...
          : browseMode === "shortcuts"
            ? (() => {
                ...
                const shortcutTagNames = new Set(shortcutTags);
                return scoped.filter(
                  (note) =>
                    shortcutIds.has(note.id) ||
                    shortcutNotebookNames.has(note.notebook) ||
  ...
  const allTags = useMemo(() => {
    return Array.from(new Set(activeNotes.flatMap((note) => note.tags))).sort((left, right) => left.localeCompare(right));
  }, [activeNotes]);
  ...
  useEffect(() => {
    ...
    setShortcutTags((previous) => {
      const next = previous.filter((name) => validTags.has(name));
      return next.length === previous.length ? previous : next;
    });
  }, [allTags]);
  ...
  function focusNote(noteId: string): void {
    ...
    if (
      ...
      !target.tags.some((tag) => shortcutTagSet.has(tag))
    ) {
      setBrowseMode("all");
    }
  ...
  function openStackContextMenu(stack: string, clientX: number, clientY: number): void {
    const position = clampMenuPosition(clientX, clientY);
    setStackMenu({ stack, x: position.x, y: position.y });
    setContextMenu(null);
    setNotebookMenu(null);
    setEditorContextMenu(null);
    setNoteListMenu(null);
  ...
  function addShortcutTag(rawTag: string): void {
    const normalized = rawTag.trim().replace(/^#/, "");
    if (!normalized) {
      return;
    }
    if (!allTags.includes(normalized)) {
      setToastMessage(`Tag #${normalized} does not exist yet`);
      return;
    }

    if (shortcutTagSet.has(normalized)) {
      setShortcutTagInput("");
      return;
    }

    setShortcutTags((previous) => [...previous, normalized]);
    setShortcutTagInput("");
    setToastMessage(`#${normalized} added to shortcuts`);
  }
  ...
  function assignNotebookToStack(notebook: string, stack: string): void {
    ...
    setCustomStacks((previous) => {
      if (previous.some((entry) => entry.toLowerCase() === lowerChosen)) {
        return previous;
      }
  ...
  function renameStack(oldName: string, nextName: string): boolean {
    ...
    if (renamed.toLowerCase() !== oldName.toLowerCase()) {
      const exists = stackNames.some((stack) => stack.toLowerCase() === renamed.toLowerCase());
      if (exists) {
        setToastMessage(`Stack "${renamed}" already exists`);
        return false;
      }
    }
    ...
    setCustomStacks((previous) => {
      const mapped = previous.map((stack) => (stack === oldName ? renamed : stack));
    ...
    setStackDropTarget((previous) => (previous === oldName ? renamed : previous));
    setToastMessage(`Stack "${oldName}" renamed to "${renamed}"`);
    return true;
  }

  function removeStack(stackName: string): void {
    const removedCount = Object.values(notebookStacks).filter((stack) => stack === stackName).length;
    setNotebookStacks((previous) => {
      let changed = false;
      const next: Record<string, string> = {};
      for (const [notebook, stack] of Object.entries(previous)) {
    ...
    setCustomStacks((previous) => previous.filter((stack) => stack !== stackName));
    setCollapsedStacks((previous) => {
      if (!previous.has(stackName)) {
        return previous;
      }
      const next = new Set(previous);
    ...
    setStackDropTarget((previous) => (previous === stackName ? null : previous));
    const detail = removedCount
      ? `${removedCount} ${removedCount === 1 ? "notebook" : "notebooks"} unstacked`
      : "No notebooks were assigned";
    setToastMessage(`Removed stack "${stackName}". ${detail}.`);
  }
...
<style>
body{margin:0;padding:40px;font-family:Georgia,"Times New Roman",serif;color:#1f2937;background:#ffffff;line-height:1.55}
main{max-width:760px;margin:0 auto}
pre{padding:12px;border-radius:8px;background:#f3f4f6;overflow:auto}
...
<main>${rendered}</main>
  ...
  function confirmStackRename(): void {
    if (!stackRenameDialog) {
      return;
    }
    const renamed = renameStack(stackRenameDialog.oldName, stackRenameDialog.newName);
    if (renamed) {
      setStackRenameDialog(null);
    }
  }
...
`;
  ...
  return (
    ...
    >
        ...
        {renderSidebarSection(
          ...
          shortcutNotes.length || shortcutNotebookItems.length || shortcutTags.length || allTags.length ? (
            <>
              ...
              <p className="shortcut-subhead">Tags</p>
              {shortcutTags.length ? (
                <ul className="shortcut-list">
                  {shortcutTags.map((tag) => (
                    <li key={tag} className="shortcut-row">
                      <button
                        type="button"
                        className="shortcut-item"
                        onClick={() => {
                      ...
                      <button
                        type="button"
                        className="shortcut-remove"
                        aria-label={`Remove tag shortcut ${tag}`}
                        onClick={() => removeShortcutTag(tag)}
                      >
                        ×
                      </button>
                    </li>
                ...
                <p className="shortcut-empty">No tag shortcuts yet</p>
              )}
              <form
                className="shortcut-add-form"
                onSubmit={(event) => {
                  event.preventDefault();
                  addShortcutTag(shortcutTagInput);
                }}
              >
                <input
                  value={shortcutTagInput}
                  placeholder="tag"
                  aria-label="Add tag shortcut"
                  onChange={(event) => setShortcutTagInput(event.target.value)}
                />
          ...
          <>
            ...
            {selectedVisibleNoteIds.length > 1 ? (
              <div className="bulk-actions" role="toolbar" aria-label="Bulk note actions">
                ...
                <button type="button" onClick={() => void copyNotesHtml(selectedVisibleNoteIds)}>
                  Copy HTML
                </button>
                      ...
                      {group.notes.map((note) => {
                        ...
                        const noteDateLabel =
                          browseMode === "trash" && note.trashedAt
                            ? `Trashed ${formatRelativeTime(note.trashedAt)}`
                        ...
                        return (
                          ...
                          >
                            ...
                            <footer>
                              <span className="note-card-date">{noteDateLabel}</span>
                              {viewMode !== "list" ? <span className="note-card-notebook">{note.notebook}</span> : null}
                            </footer>
                            {viewMode !== "list" && (note.reminderAt || note.isTemplate || homePinned || notebookPinned) ? (
                              <div className="note-card-chips" aria-label="Note markers">
                                {note.reminderAt ? (
      ...
      {stackMenu ? (
        ...
        >
          <button
            type="button"
            onClick={() => {
              setStackRenameDialog({ oldName: stackMenu.stack, newName: stackMenu.stack });
              setStackMenu(null);
            }}
          >
      ...
      {createStackDialog ? (
        <div className="overlay" onClick={() => setCreateStackDialog(null)}>
          <section className="rename-modal" onClick={(event) => event.stopPropagation()}>
            <h3>New stack</h3>
            <input
              autoFocus
              placeholder="Stack name"
              value={createStackDialog.name}
              onChange={(event) => setCreateStackDialog({ name: event.target.value })}
              onKeyDown={(event) => {
                if (event.key === "Enter") {
                  event.preventDefault();
                  saveCreateStackDialog();
                }
              }}
            />
            <footer>
              <button type="button" onClick={() => setCreateStackDialog(null)}>
                Cancel
              </button>
              <button type="button" className="primary" onClick={saveCreateStackDialog}>
                Create
              </button>
            </footer>
          </section>
      ...
      {stackRenameDialog ? (
        <div className="overlay" onClick={() => setStackRenameDialog(null)}>
          <section className="rename-modal" onClick={(event) => event.stopPropagation()}>
            <h3>Rename stack</h3>
            <input
              value={stackRenameDialog.newName}
              onChange={(event) => setStackRenameDialog({ ...stackRenameDialog, newName: event.target.value })}
              onKeyDown={(event) => {
                if (event.key === "Enter") {
                  event.preventDefault();
                  confirmStackRename();
            ...
            />
            <footer>
              <button type="button" onClick={() => setStackRenameDialog(null)}>
                Cancel
              </button>
              <button type="button" className="primary" onClick={confirmStackRename}>
                Save
              </button>
            </footer>
          </section>
